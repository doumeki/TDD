...
class HomepageTest(TestCase):
    ...
    def _test_home_page_can_save_a_post_request(self):
        request = HttpRequest()
        request.method = 'POST'
    ...
    def test_home_page_can_save_a_POST_request(self):
        ...
        # response = home_page(request)
        #
        # self.assertEqual(Item.objects.count(),1) #代表本次ITEM存入的值
        ...
        #
        # # self.assertIn('A new list item',response.content.decode())
        # # expcted_html = render_to_string('home.html',{'new_item_text':'A new list item'},request)
        # #
        # # rep = HomepageTest.remove_csrf(response.content.decode())
        # # expect_html = HomepageTest.remove_csrf(expcted_html)
        # #
        # # self.assertEqual(rep,expect_html)
        pass
...
class ItemModelTest(TestCase):
    ...
    def _test_home_page_displays_all_list_items(self):
...
class ListViewTest(TestCase):
    def test_displays_only_items_for_that_list(self):
        ...
        #
        # response = self.client.get('/lists/the-only-list-in-the-world/') #直接调用URL的方式，而不是调用方法
        #
        # self.assertContains(response,'itemey 1') #response不用转码了，因为使用了assertContains
...
class NewListTest(TestCase):
    ...
    def test_redirects_after_POST(self):
        ...
        # self.assertRedirects(response,'/lists/%d/'%(new_list.id,),fetch_redirect_response=False) #要加最后一个参数才不报错
